...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    ...
    let rows = file
        ...
        .iter()
        .flat_map(|list| {
            let name = if list.blocked() { "Blocked" } else { "Allowed" };
    ...
    let identity = vec![
        ("Lists", file.lists().len().to_string()),
        ("Words", rows.len().to_string()),
        ("Blocked", count("Blocked").to_string()),
        ("Allowed", count("Allowed").to_string()),
        ("Dropped", file.skipped().len().to_string()),
        ("Folded", file.replacements().len().to_string()),
    ];
    ...
    pub fn details_ui(&self, ui: &mut egui::Ui) {
        ScrollArea::vertical().auto_shrink(false).show(ui, |ui| {
            ...
            if !self.dropped.is_empty() {
                section(ui, "Dropped");
                ui.label(
                    RichText::new("Characters taken out of a phrase before it is matched.").weak(),
                );
            ...
            if !self.folded.is_empty() {
                section(ui, "Folded");
                ui.label(RichText::new("Characters read as another before matching.").weak());
...
mod tests {
    ...
    fn searches_the_words_it_read() {
        ...
        assert_eq!(
            file.rows,
            [("Blocked", "ab".to_owned()), ("Blocked", "ac".to_owned())]
        );
