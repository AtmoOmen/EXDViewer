...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    ...
    let identity = vec![
        ("字号", format!("{} px", font.size())),
        ("行高", font.line_height().to_string()),
        ("上升高度", font.ascent().to_string()),
        ("下降高度", font.descent().to_string()),
        ("字形", font.glyphs().len().to_string()),
        ("字偶距组合", font.kerning().len().to_string()),
        (
            "纹理表",
    ...
    if font.blocks.len() > 1 {
        ui.horizontal_wrapped(|ui| {
            for (block, count) in &font.blocks {
                let name = BLOCKS.get(*block).map_or("其他", |(name, _, _)| name);
                let bit = 1 << block;
                if ui
