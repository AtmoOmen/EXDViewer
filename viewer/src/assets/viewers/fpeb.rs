...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    let buf = Buf(bytes);
    (bytes.get(4..8) == Some(b"FPEB")).then_some(()).context("面部参数编辑文件")?;
    let root = buf.table(0).context("根表")?;
    ...
    let identity = vec![
        ("体型", bodies.to_string()),
        ("参数", rows.len().to_string()),
        (
            "保持原样",
            rows.iter().filter(|(.., scale)| *scale == 1.0).count().to_string(),
        ),
    ];
    log::info!("assets/fpeb: {path} {bodies} 个体型，{} 个参数", rows.len());
...
pub fn ui(ui: &mut egui::Ui, file: &Rendered) {
    ...
    ui.label(
        RichText::new(
            "这两根骨骼都没有动画驱动，其尺寸完全由本表决定。某个体型、脸型或眼型在表中没有条目时，\
             骨骼保持初始大小。",
        )
        .weak(),
    );
