...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    ...
    let identity = vec![
        ...
        ("模型", model(file.model_type(), file.model_id())),
        ("模型类型", model_type(file.model_type())),
        ("变体", file.variant().to_string()),
        ("动画", animations.len().to_string()),
        (
            "动作",
            match &motions {
...
pub fn ui(ui: &mut egui::Ui, file: &Rendered, backend: &Backend) -> Option<String> {
    let mut follow = None;
    ui.horizontal(|ui| {
        for (tab, label) in [(Tab::Animations, "动画"), (Tab::Playback, "播放")] {
    ...
    let Some(binding) = file
        ...
        ui.label(RichText::new("此动画未指定动作" ).weak());
        return follow;
    };
