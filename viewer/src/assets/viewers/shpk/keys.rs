...
pub fn read(package: &shpk::ShaderPackage) -> Keys {
    ...
    let groups: Vec<(&'static str, Vec<KeyRow>)> = [
        ("系统", package.system_keys()),
        ("场景", package.scene_keys()),
        ("材质", package.material_keys()),
    ]
    .into_iter()
    .map(|(group, list)| {
        let rows = list
...
impl Keys {
    ...
    pub fn defines_ui(&self, ui: &mut egui::Ui, shader: usize) {
        ...
        if let Some((count, complete)) = self.selected.get(shader) {
            let note = match complete {
                ...
                false => format!("{count} 种组合，部分搭配不会出现"),
            };
            ...
            .show(ui, |ui| {
                for (column, key) in self.columns.iter().enumerate() {
                    let held = defines
                        ...
                        .find(|(at, _)| *at == column)
                        .map(|(_, value)| *value);
                    hashed(ui, "键", &key.name, key.id, held.is_none());
                    match held {
                        Some(value) => {
                        ...
                        // about it.
                        None => {
                            ui.label(RichText::new("任意").weak().small());
                        }
                    }
                    ui.end_row();
                }
    ...
    pub fn ui(&self, ui: &mut egui::Ui) {
        for (group, rows) in &self.groups {
            ...
            heading(ui, group);
            for key in rows {
                hashed(ui, "键", &key.name, key.id, false);
