...
pub fn ui(ui: &mut egui::Ui, package: &Rendered, bytes: &[u8]) {
    ...
    ui.horizontal_wrapped(|ui| {
        if ui
            .selectable_label(stage == 0, format!("全部（{}）", package.shaders.len()))
            .clicked()
        {
            stage = 0;
        }
    ...
    if !package.keys.passes.is_empty() {
        ui.add_space(2.0);
        ui.horizontal_wrapped(|ui| {
            if ui.selectable_label(pass == 0, "任意渲染通道").clicked() {
                pass = 0;
            }
    ...
    if package.keys.columns.iter().any(|key| key.values.len() > 1) {
        ...
        egui::CollapsingHeader::new(title)
            ...
            .show(ui, |ui| {
                    ...
                    .show(ui, |ui| {
                        for (at, (index, key)) in filterable.iter().enumerate() {
                            ...
                            let selected = held
                                ...
                                .map_or("任意", |(short, _)| short.as_str());
                ...
                if set > 0 && ui.small_button("清除").clicked() {
...
fn tooltip(ui: &mut egui::Ui, package: &Rendered, index: usize) {
    ...
    if !reached.is_empty() {
        ui.add_space(4.0);
        ui.label(RichText::new("用于绘制").weak().small());
        for pass in reached {
    ...
    if !shader.bindings.is_empty() {
        ui.add_space(4.0);
        ui.label(RichText::new("绑定资源").weak().small());
    ...
    if !set.is_empty() {
        ui.add_space(4.0);
        ui.label(RichText::new("编译条件").weak().small());
