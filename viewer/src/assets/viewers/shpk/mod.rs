...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    ...
    let identity = vec![
        ...
        ("着色器", package.shaders().len().to_string()),
        ("字节码", Bytes(package.bytecode_size()).to_string()),
        (
            "参数缓冲区",
            format!(
                "{} 个寄存器（{} B）",
        ...
        ("选择器节点", package.nodes().len().to_string()),
        ("别名", package.aliases().len().to_string()),
        ("子视图 1", shader::named(subview_one)),
        ("子视图 2", shader::named(subview_two)),
    ];
