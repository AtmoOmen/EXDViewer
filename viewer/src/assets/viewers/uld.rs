...
fn component_kind(kind: uld::ComponentKind) -> String {
    match kind {
        uld::ComponentKind::Base => "基础组件",
        uld::ComponentKind::Button => "按钮",
        uld::ComponentKind::Window => "窗口",
        uld::ComponentKind::CheckBox => "复选框",
        uld::ComponentKind::RadioButton => "单选按钮",
        uld::ComponentKind::GaugeBar => "量表",
        uld::ComponentKind::Slider => "滑块",
        uld::ComponentKind::TextInput => "文本输入框",
        uld::ComponentKind::NumericInput => "数值输入框",
        uld::ComponentKind::List => "列表",
        uld::ComponentKind::DropDownList => "下拉列表",
        uld::ComponentKind::Tab => "页签",
        uld::ComponentKind::TreeList => "树形列表",
        uld::ComponentKind::ScrollBar => "滚动条",
        uld::ComponentKind::ListItemRenderer => "列表项渲染器",
        uld::ComponentKind::Icon => "图标",
        uld::ComponentKind::IconText => "图标文本",
        uld::ComponentKind::DragDrop => "拖放区域",
        uld::ComponentKind::GuildLeveCard => "理符卡片",
        uld::ComponentKind::TextNineGrid => "文本九宫格",
        uld::ComponentKind::JournalCanvas => "任务日志画布",
        uld::ComponentKind::Multipurpose => "多用途组件",
        uld::ComponentKind::Map => "地图",
        uld::ComponentKind::Preview => "预览",
        uld::ComponentKind::HoldButton => "长按按钮",
        uld::ComponentKind::Portrait => "肖像",
        uld::ComponentKind::XbmItem => "XBM 项目",
        uld::ComponentKind::XbmContentStageEventMap => "XBM 内容阶段事件映射",
        uld::ComponentKind::Unknown(_) => "未知组件",
    }
    .to_owned()
}

fn describe(node: &uld::Node, layout: &uld::UiLayout) -> (String, String) {
    let part = |list: u32, part: u32| format!("列表 {list} 的部件 {part}");
    match node.kind() {
        NodeKind::Res => ("资源".to_owned(), String::new()),
        NodeKind::Image(image) => ("图像".to_owned(), part(image.part_list_id, image.part_id)),
        NodeKind::Text(text) => (
            "文本".to_owned(),
            format!("文本 {}，{:?} {}", text.text_id, text.font, text.font_size),
        ),
        NodeKind::NineGrid(grid) => ("九宫格".to_owned(), part(grid.part_list_id, grid.part_id)),
        NodeKind::Counter(counter) => (
            "计数器".to_owned(),
            format!("列表 {}", counter.part_list_id),
        ),
        NodeKind::Collision(_) => ("碰撞".to_owned(), String::new()),
        NodeKind::ClippingMask(mask) => {
            ("裁剪遮罩".to_owned(), part(mask.part_list_id, mask.part_id))
        }
        ...
        NodeKind::Component { component_id, .. } => (
            layout
                ...
                .map_or_else(|| "组件".to_owned(), |c| component_kind(c.kind())),
            format!("组件 {component_id}"),
        ),
        NodeKind::Unknown { node_type, .. } => (format!("类型 {node_type}"), "未建模".to_owned()),
    }
}
...
fn properties(node: &uld::Node, layout: &uld::UiLayout, rect: Rect) -> Vec<(&'static str, String)> {
    let mut props = vec![
        ("节点", node.id().to_string()),
        ("类型", node.node_type().to_string()),
        ("位置", format!("{}, {}", node.x(), node.y())),
        ("尺寸", format!("{} x {}", node.width(), node.height())),
    ];
    ...
    let flags = [
        ...
        (flags.clip(), "裁剪子节点"),
        (flags.fill(), "填充父节点"),
        (flags.anchor_top(), "锚定上边缘"),
        (flags.anchor_bottom(), "锚定下边缘"),
        (flags.anchor_left(), "锚定左边缘"),
        (flags.anchor_right(), "锚定右边缘"),
        (flags.has_collision(), "包含碰撞区域"),
    ]
    .into_iter()
    ...
    props.push(("标志", flags));
    if node.parent_id() != 0 {
        props.push(("父节点", node.parent_id().to_string()));
    }
    if node.alpha() != 255 {
        props.push(("透明度", node.alpha().to_string()));
    }
    let [mr, mg, mb] = node.multiply();
    if [mr, mg, mb] != [100, 100, 100] {
        props.push(("乘色", format!("{mr}, {mg}, {mb}")));
    }
    let [ar, ag, ab] = node.add();
    if [ar, ag, ab] != [0, 0, 0] {
        props.push(("加色", format!("{ar}, {ag}, {ab}")));
    }
    if node.rotation() != 0.0 {
        props.push(("旋转", format!("{:.3}", node.rotation())));
    }
    ...
    if (node.origin_x(), node.origin_y()) != (0, 0) {
        props.push(("原点", format!("{}, {}", node.origin_x(), node.origin_y())));
    }
    if node.timeline_id() != 0 {
        props.push(("时间轴", node.timeline_id().to_string()));
    }

    let part = |list: u32, id: u32| {
        let sprite = sprite_of(layout, list, id);
        let where_ = format!("列表 {list} 的部件 {id}");
        match sprite {
            Some(sprite) => vec![
                ("部件", where_),
                (
                ...
                (
                    "纹理",
                    sprite.texture.unwrap_or_else(|| "未声明".to_owned()),
                ),
            ],
            None => vec![("部件", format!("{where_}（缺失）"))],
        }
    };
    ...
    match node.kind() {
        NodeKind::Image(image) => {
            ...
            props.push(("环绕", image.wrap.to_string()));
        }
        ...
        NodeKind::Counter(counter) => {
            ...
            props.push(("数字", format!("宽度 {}", counter.number_width)));
        }
        NodeKind::Text(text) => {
            props.push((
                "文本",
            ...
            props.push(("对齐", text.alignment.to_string()));
            let flags = [
                (text.flags.bold(), "粗体"),
                (text.flags.italic(), "斜体"),
                (text.flags.edge(), "描边"),
                (text.flags.glare(), "光泽"),
                (text.flags.multiline(), "多行"),
                (text.flags.ellipsis(), "省略号"),
                (text.flags.word_wrap(), "自动换行"),
                (text.flags.emboss(), "浮雕"),
            ]
            .into_iter()
            .filter_map(|(enabled, name)| enabled.then_some(name))
            .collect::<Vec<_>>()
        ...
        NodeKind::Component {
            ...
            props.push((
                ...
                match layout.component(*component_id) {
                    Some(component) => {
                        format!("{component_id}（{}）", component_kind(component.kind()))
                    }
                    None => format!("{component_id}（未定义）"),
                },
            ));
            props.push(("实例", format!("{:?}", instance.index)));
        }
        NodeKind::Unknown { node_type, data } => {
            props.push(("载荷", format!("类型 {node_type}，{} 字节", data.len())));
        }
...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    ...
    let widgets = layout
        ...
        .map(|widget| {
            ...
            let alignment = match widget.alignment() {
                uld::Alignment::TopLeft => "左上",
                uld::Alignment::Top => "上方",
                uld::Alignment::TopRight => "右上",
                uld::Alignment::Left => "左侧",
                uld::Alignment::Center => "居中",
                uld::Alignment::Right => "右侧",
                uld::Alignment::BottomLeft => "左下",
                uld::Alignment::Bottom => "下方",
                uld::Alignment::BottomRight => "右下",
                uld::Alignment::Unknown(_) => "未知",
            };
    ...
    let identity = vec![
        ("版本", format!("{:?}", layout.version())),
        ("纹理", layout.textures().len().to_string()),
        ("部件列表", layout.part_lists().len().to_string()),
        ("组件", layout.components().len().to_string()),
        ("控件", layout.widgets().len().to_string()),
    ];
        ...
        .show(ui, |ui| {
            ...
            if !layout.part_lists.is_empty() {
                ...
                for list in &layout.part_lists {
                    ui.label(
                        RichText::new(format!("列表 {}（{} 个部件）", list.id, list.parts.len()))
                            .weak(),
                    );
...
pub fn details_ui(ui: &mut egui::Ui, layout: &Rendered, deps: &mut Deps, backend: &Backend) {
    ...
    ScrollArea::vertical().auto_shrink(false).show(ui, |ui| {
                ...
                .show(ui, |ui| {
                    for (index, widget) in layout.widgets.iter().enumerate() {
                        if layout.widgets.len() > 1 {
                            ui.label(RichText::new(format!("控件 {}", widget.id)).weak());
                        }
                        tree(
                            ui,
                            &mut toggles,
        ...
        if !layout.timelines.is_empty() {
            ui.add_space(8.0);
            ui.separator();
            ui.label(RichText::new("时间轴").weak());
            ui.add_space(4.0);
            egui::Grid::new("uld_timelines")
                ...
                .show(ui, |ui| {
                    for (id, animations, label_sets, groups) in &layout.timelines {
                        ...
                        ui.label(RichText::new(format!("{groups} 个关键帧组")).weak());
                        ui.label(
                            RichText::new(match label_sets {
                                0 => String::new(),
                                _ => format!("{label_sets} 个标签集"),
                            })
                            .weak(),
                        );
                        ui.end_row();
