...
const COLUMNS: [(&str, usize); 3] = [("起始套装", 9), ("代码", 6), ("目录", 0)];
...
pub fn decode(path: &str, bytes: &[u8]) -> Result<Preview> {
    let rows = types(bytes).context("武器类型表")?;
    let distinct = {
    ...
    log::info!("assets/wtd: {path} {} 个条目，{distinct} 个代码", rows.len());
...
pub fn ui(ui: &mut egui::Ui, file: &Rendered) {
    ...
    ui.label(
        RichText::new(
            "每一行给出从该套装起、直到下一行之前的套装所读取的代码，因此位于两行之间的套装\
             取靠下那一行的代码。",
        )
        .weak(),
    );
