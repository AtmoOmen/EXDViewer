...
/// so every drawn weapon shares this one until that is resolved; every other race lacks `a0034`
/// entirely, so the caller carries a fallback to whichever stance was already on screen.
