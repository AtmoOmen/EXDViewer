...
mod imp {
    ...
    fn origin() -> Result<String> {
        window()
            .ok_or_else(|| anyhow!("未找到浏览器窗口"))?
            .location()
            .origin()
            .map_err(|e| anyhow!("无法获取来源: {e:?}"))
    }
    ...
    fn local_storage() -> Result<web_sys::Storage> {
        window()
            .ok_or_else(|| anyhow!("未找到浏览器窗口"))?
            .local_storage()
            .map_err(|e| anyhow!("localStorage 不可用: {e:?}"))?
            .ok_or_else(|| anyhow!("localStorage 不可用"))
    }
