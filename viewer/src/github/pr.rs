...
impl GithubClient {
    ...
    async fn call_json(&self, method: &str, url: &str, body: Option<&Value>) -> Result<Value> {
        ...
        if !(200..300).contains(&resp.status) {
            ...
            bail!(
                "GitHub API {method} {url} 请求失败 ({}): {message}",
                resp.status
            );
        }
    ...
    pub async fn submit_pr(&self, draft: &PrDraft) -> Result<PrResult> {
        ...
        Ok(PrResult {
            ...
            number: pr
                ...
                .ok_or_else(|| anyhow!("PR 响应缺少 number 字段"))?,
        })
    }
        ...
        for attempt in 0..10u32 {
            ...
            if resp.status != 422 {
                ...
                let message = json
                    ...
                    .map(str::to_owned)
                    .unwrap_or_else(|| resp.text());
                bail!("创建分支失败 ({}): {message}", resp.status);
            }
        }
        bail!("未找到可用的分支名称")
    }
    ...
    async fn wait_for_fork(&self, fork_url: &str) -> Result<()> {
        ...
        for attempt in 0..MAX_ATTEMPTS {
            ...
            if resp.status != 404 {
                bail!("访问派生仓库失败 ({}): {}", resp.status, resp.text());
            }
            log::debug!("正在等待派生仓库可用 (第 {attempt} 次尝试)");
            yield_to_ui().await;
        }
        bail!("派生仓库未能及时可用，请重试")
    }
...
fn str_field<'a>(value: &'a Value, key: &str) -> Result<&'a str> {
    value
        .get(key)
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("GitHub 响应缺少字符串字段 '{key}'"))
}
...
fn nested_str<'a>(value: &'a Value, path: &[&str]) -> Result<&'a str> {
    ...
    for key in path {
        current = current
            .get(key)
            .ok_or_else(|| anyhow!("GitHub 响应缺少字段 '{key}'"))?;
    }
    current
        .as_str()
        .ok_or_else(|| anyhow!("GitHub 响应字段 '{}' 不是字符串", path.join(".")))
}
