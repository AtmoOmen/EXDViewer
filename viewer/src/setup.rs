...
impl SetupWindow {
    pub fn from_blank(is_startup: bool) -> Self {
        ...
        let location = ironworks::sqpack::Install::search()
            ...
            .unwrap_or(InstallLocation::Web(Region::China, None));

        #[cfg(target_arch = "wasm32")]
        let location = InstallLocation::Web(Region::China, None);
                        ...
                        ui.horizontal(|ui| {
                            ui.columns_const(|[col_0, col_1]| {
                                ...
                                if radio(
                                    ...
                                    self.location = InstallLocation::Web(Region::China, None);
                                }
                            });
                        });
            ...
            match result {
                Ok(handle) => {
                    let handle = handle
                        ...
                        .map_err(|_| anyhow::anyhow!("无法转换为 FileSystemDirectoryHandle"))?;
                    let handle = WorkerDirectory(handle);
                    store_folder(handle.clone()).await.map(|()| handle)
                }
                Err(e) => Err(anyhow::anyhow!("选择文件夹失败: {e:?}")),
            }
        });
