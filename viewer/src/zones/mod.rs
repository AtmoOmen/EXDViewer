...
impl ZoneBrowser {
    ...
    pub fn open_palette(&mut self) {
        self.palette = Some(Palette::new("查找场景…", "筛选", self.search.clone()));
    }
    ...
    fn poll_open(&mut self) {
        ...
        opened.state = match result {
            Ok(bytes) => match layer::lvb::decode(&opened.path, bytes) {
                ...
                Ok(_) => unreachable!("lvb 解码必然得到图层预览"),
                ...
                Panel::top("zone_list_header").show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.with_layout(Layout::right_to_left(Align::Min), |ui| {
                            ...
                            ui.vertical_centered_justified(|ui| ui.heading("场景"));
                        });
                    });
                    ui.add_space(4.0);
                    ui.with_layout(Layout::right_to_left(Align::Min), |ui| {
                        ...
                        if unavailable > 0 {
                            ui.toggle_value(&mut self.show_unavailable, "🚫")
                                .on_hover_text(format!("显示 {unavailable} 个不可用场景"));
                        }
                        ui.add_sized(
